...
module Iterable
  class UpsertInAppTemplateModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Iterable::UpsertInAppTemplateModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Iterable::UpsertInAppTemplateModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [client_template_id, name, html, in_app_display_settings, web_in_app_display_settings, payload, expiration_date_time, expiration_duration, locale, message_type_id, creator_user_id, message_medium].hash
    end
