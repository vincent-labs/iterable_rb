  ...
  class InAppContentApi
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Iterable::InAppContentApi` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Iterable::InAppContentApi`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @web_in_app_display_settings.nil?
        invalid_properties.push('invalid value for "web_in_app_display_settings", web_in_app_display_settings cannot be nil.')
      end
    ...
    def valid?
      return false if @web_in_app_display_settings.nil?
      true
    end
    ...
    def hash
      [html, in_app_display_settings, web_in_app_display_settings].hash
    end
