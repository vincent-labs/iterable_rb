...
module Iterable
  class UpsertEmailTemplateModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Iterable::UpsertEmailTemplateModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Iterable::UpsertEmailTemplateModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @client_template_id.nil?
        invalid_properties.push('invalid value for "client_template_id", client_template_id cannot be nil.')
      end
    ...
    def valid?
      return false if @client_template_id.nil?
    ...
    def hash
      [client_template_id, name, from_name, from_email, reply_to_email, subject, preheader_text, cc_emails, bcc_emails, html, plain_text, google_analytics_campaign_name, link_params, data_feed_id, data_feed_ids, cache_data_feed, merge_data_feed_context, locale, message_type_id, creator_user_id, message_medium].hash
    end
